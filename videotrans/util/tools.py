...
def set_proxy(set_val=''):
    print('set_proxy, set_val is ', set_proxy)
    if set_val == 'del':
        # 删除代理
...
def runffmpegbox(arg):
    ...
    cmd_ok = ' '.join(cmd)
    print('run ffmpeg box  cmd_ok is {}'.format(cmd_ok))
...
def runffmpeg(arg, *, noextname=None, error_exit=True):
    ...
    cmd = cmd + arg
    cmd_ok = ' '.join(cmd)
    print('run ffmpeg cmd_ok is {}'.format(cmd_ok))
    if noextname:
...
def get_lastjpg_fromvideo(file_path, img):
    return runffmpeg(
        ['-y', '-sseof', '-3', '-i', f'{file_path}', '-vsync', '0', '-q:v', '1', '-qmin:v', '1', '-qmax:v', '1',
         '-update', 'true', f'{img}'])
